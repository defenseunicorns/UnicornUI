<script lang="ts">
  ...
  import { updateDataTheme, updateThemeStyle } from '../shared/theme/config/theme-config.utils';
  ...
  import type { Palettes, SupportedThemes } from '../shared/theme/palette/palette.types';
  ...
  export let theme: SupportedThemes = 'light';
  ...
  let watchedTheme: SupportedThemes;
  ...
  onMount(() => {
    ...
    const themePreference = (preferredTheme && getPreferredTheme(window)) || theme;
  ...
  // Update the data-theme variable when theme changes.
  $: updateDataTheme(pageDocument, watchedTheme);
</script>
